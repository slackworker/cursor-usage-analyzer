...
**最后更新**：2026-06-26（Total spend 模型 + January/February 校准）
...
后续将追加 March、April、May 等月度样例（命名约定：`{Month} - US${total}.csv`）。
...
详见 [`cursor_usage/pricing.py`](../cursor_usage/pricing.py)。校准来源：Cursor 官方文档 + January/February 2026 导出。
...
**M0 状态：通过**（January + February 与官方几乎对齐），可进入 M1。
...
```
M0 冒烟 + spec（January/February）  ← 当前
     → 追加 March / April / May 样例与 golden
